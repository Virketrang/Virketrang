import { Input, Select } from '@/nexus/client/components'
...
const ProductionUnits = () => {
    ...
    return (
        <>
            <div flex flex-column align-center gap-2>
            ...
            <div class="stepper" role="progressbar" flex gap-1>
                <For each={store().production_units}>
                    {(_, i) => (
                        <button active={index() === i()} onClick={() => setIndex(i)} elevated circle size-2>
                            {i() + 1}
                        </button>
                    )}
                </For>

                <button elevated size-2 circle onClick={add}>
                    +
                </button>
            </div>
        </>
